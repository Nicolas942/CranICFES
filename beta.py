...
while corriendo:
    ...
    boton_silencio = pygame.Rect(100, 100, 200, 50) 
    ...
    if pantalla_actual == "menu":
        ...
        ventana.blit(texto, (ventana.get_width() // 2 - texto.get_width() // 2, ventana.get_height() // 2 - 60))
        pygame.draw.rect(ventana, ROJO, boton_salir)

        # silenciar/reactivar música
        pygame.draw.rect(ventana, CELESTE, boton_silencio)
        font_boton = pygame.font.SysFont(None, 30)
        texto_boton = font_boton.render("Silenciar" if musica_activa else "Reactivar", True, NEGRO)
        ventana.blit(texto_boton, (boton_silencio.x + 20, boton_silencio.y + 10))
